...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  const item = await prisma.risk.findUnique({ where: { id: params.id } });
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  const parsed = RiskUpdateSchema.safeParse(json);
  if (!parsed.success) {
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  const deleted = await prisma.risk.delete({ where: { id: params.id } }).catch(() => null);
